...
public class SeleccionPaquetes {
    ...
    private static int auxFuerzaBruta(int[] costos, int[] ganancias, int presupuesto, int index) {
        ...
        int opcion1 = auxFuerzaBruta(costos, ganancias, presupuesto, index + 1);

        int opcion2 = 0;
        if (costos[index] <= presupuesto) {
            opcion2 = ganancias[index] + auxFuerzaBruta(costos, ganancias, presupuesto - costos[index], index + 1);
        }
